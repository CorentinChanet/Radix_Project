...
class RegxSections(Enum):
    work_exp = r'((teaching|exp.{3,4}nce|employment|work|working|career|job.?|professional|organizational|industr.{1,3}|previous.{0,2}|present.{0,2}|current.{0,2}|detail.{0,2})' \
           r'.{0,4}' \
           r'(record.?|role.?|responsibilit.{1,4}|exposure|synopsis|detail.?|exp.{3,4}nce|employ.{2,4}|overview|background|highlight|summar.{1,3}|histor.{0,4}|position.?|ap.?oint.?ment.?)' \
           r'|^.{0,6}internship|^.{0,6}experience|^.{0,6}career.{0,4}$|^.{0,6}job.{0,5}$|^.?personal experience$|^.{0,6}responsibilit.{1,6}$|^.{0,6}dut.{1,3}|academic.?.{1,4}achievement.?)'
    ...
    skills = r'(techn.{2,8}|core|key|area.?|professional|additional|organization.{0,2}|software|it|industry|personal.{0,3}|computer|relevant|hard.{0,5}|soft|practical).{0,4}(skil.{1,2}|trait.?|knowledge|strength.?|proficiency|competenc.{1,4}|capabilit.{1,4}|capacit.{1,4}|abilit.{1,4}|experti.e|literacy)|^.?skill.{0,4}|^.?strength.{0,4}|^.?proficienc.{1,3}'
...
class RegxInfos(Enum):
    ...
    phone_numbers = r"((?:\+\d{2}[-\.\s]??|\d{4}[-\.\s]??)?(?:\d{3}[-\.\s]??\d{3}[-\.\s]??\d{4}|\(\d{3}\)\s*\d{3}[-\.\s]??\d{4}|\d{3}[-\.\s]??\d{4}))"
